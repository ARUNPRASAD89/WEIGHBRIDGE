...
from PyQt5.QtCore import Qt, QDateTime, QDate, QTime, QLocale, pyqtSlot, QTimer
...
from serial_manager import SerialManager, get_serial_manager
...
class BaseTransactionWindow(QDialog):
    def __init__(self, parent=None):
        ...
        # --- SERIAL PORT INTEGRATION (shared manager) ---
        self.serial_manager = get_serial_manager()
        # Connect signals once (guard so multiple windows don't re-connect)
        try:
        ...
        # Acquire shared manager (starts it when first window acquires)
        try:
            self.serial_manager.acquire()
        except Exception:
            pass

        # Acquire the shared serial manager (this will start it if not already running)
        self.serial_manager.acquire()
    ...
    def stop_serial_manager(self):
        """Release the shared serial manager."""
        if hasattr(self, 'serial_manager'):
            try:
    ...
    def return_to_main_menu(self):
        """ Handles returning to the main menu from any transaction window. """
        # Find the main TransactionTypeWindow, which is the ultimate parent
        main_menu = self
        while main_menu.parent() and not isinstance(main_menu.parent(), QWidget):
             # This loop goes up the chain to find the top-level window if needed
             # but usually self.parent() is the one we want.
             p = main_menu.parent()
             if p is None: break
             main_menu = p
        
        # If the direct parent exists (it should), show it.
        if self.parent():
             self.parent().show()
    ...
    def set_active_transaction_buttons(self, mode):
        """
        Controls which of the transaction type buttons are enabled.
        mode: 'base' (all enabled), 'single', 'first', 'second'
        """
        is_base = (mode == 'base')
        is_single = (mode == 'single')
        is_first = (mode == 'first')
        is_second = (mode == 'second')

        self.btn_single.setEnabled(is_base or is_single)
        self.btn_first.setEnabled(is_base or is_first)
        self.btn_second.setEnabled(is_base or is_second)
