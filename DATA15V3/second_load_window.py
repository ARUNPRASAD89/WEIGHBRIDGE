...
class SummaryDialog(QDialog):
    def __init__(self, parent, data, transaction_window=None):
        ...
        self.print_btn.clicked.connect(self.on_weightprint)
    ...
    def on_weightprint(self):
        try:
            # Get all fields from self.data (from summary_data)
            ticket_number = self.data.get('TicketNumber', '').strip()
            vehicle_number = self.data.get('VehicleNumber', '').strip()
            date = self.data.get('Date', '').strip()
            time = self.data.get('Time', '').strip()
            empty_weight = self.data.get('EmptyWeight', '').strip()
            loaded_weight = self.data.get('LoadedWeight', '').strip()
            net_weight = self.data.get('NetWeight', '').strip()
            eamount = self.data.get('EAMOUNT', '').strip()
            lamount = self.data.get('LAMOUNT', '').strip()
            tamount = self.data.get('TAMOUNT', '').strip()

            # --- Use helper as in second_transaction_window.py ---
            def blank_to_none(val):
                return None if val in ("", None) else int(val)

            # Fetch DB pending/closed status
            ...
            params = {
                ...
                "Date": date,
                "Time": time,
                "EmptyWeight": blank_to_none(empty_weight),
                "LoadedWeight": blank_to_none(loaded_weight),
                "EmptyWeightDate": date,
                "EmptyWeightTime": time,
                "LoadWeightDate": date,
                "LoadWeightTime": time,
                "NetWeight": blank_to_none(net_weight),
                "Pending": pending,
                "Closed": closed,
                "Exported": False,
                "Shift": "B",
                ...
                "AMOUNT": None,
                "STATUS": "",
                "EAMOUNT": blank_to_none(eamount),
                "LAMOUNT": blank_to_none(lamount),
                "TAMOUNT": blank_to_none(tamount),
                "NetWeight1": None,
                "LWEIGHT": None,
                "EWEIGHT": None,
            }
        ...
        # Prepare ticket_data (for printing)
            ...
            self.ticket_data = ticket_data  # for printing
            ...
            self._print_timer.timeout.connect(self._do_print_ticket)
            self._print_timer.start(3000)
    ...
    def _do_print_ticket(self):
        try:
            ...
            conn = get_connection()
            print_ticket_with_template(self.ticket_data, conn)

            # After print, return to transaction window (or parent) after 5 seconds
...
class SecondLoadWindow(QWidget):
    ...
    def ok_pressed(self):
        ...
        if data:
        # >>> ADD THIS BLOCK <<<
            if (str(data.get("Pending")).lower() in ["false", "f", "0", "", "none", "null"] or not data.get("Pending")) and \
               (str(data.get("Closed")).lower() in ["true", "t", "1"] or data.get("Closed")):
                QMessageBox.warning(self, "Ticket Closed", "Ticket cannot be overwritten.")
