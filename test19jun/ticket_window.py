from PyQt5.QtWidgets import (
    ...
    QVBoxLayout, QGridLayout, QFrame, QMessageBox, QInputDialog
)
...
class TicketWindow(QWidget):
    def __init__(self):
        ...
        self.op_buttons[0].clicked.connect(self.handle_weigh)  # Weigh
        self.op_buttons[1].clicked.connect(self.handle_save)   # Save
    ...
    def handle_weigh(self):
        # Only for single transaction, as per your instruction
        if self.title_label.text().lower().find("single") != -1:
            # Simulate getting live weight from a scale (replace with real reading code if available)
            live_weight, ok = QInputDialog.getInt(self, "Simulate Weigh", "Enter live weight (kg):", min=0)
            if ok:
                # For demo, set Loaded Weight and update display
                self.loaded_weight.setText(str(live_weight))
                self.weight_display.setText(f"{live_weight} <span style='font-size:15pt'>kg</span>")
                # Optionally, recalculate net weight
                self.update_net_weight()
        else:
            QMessageBox.information(self, "Info", "Weighing is only active for Single Transaction mode.")
    ...
    def handle_save(self):
        ...
        ticket_data = {
            ...
            "load_status": self.load_status.currentText(),
            "material": self.material.text(),
            "supplier": self.supplier.text(),
            "loaded_weight": self.loaded_weight.text(),
            "empty_weight": self.empty_weight.text(),
            "net_weight": self.net_weight.text(),
        ...
        # Insert into ticket table (make sure the table and columns exist)
        try:
            query = """
                INSERT INTO tickets (
                    ticketnumber, vehiclenumber, emptyweight, loadedweight, netweight,
                    materialname, suppliername
                ) VALUES (%s, %s, %s, %s, %s, %s, %s)
            """
            params = (
                ticket_data["ticket_number"],
                ticket_data["vehicle_number"],
                ticket_data["empty_weight"],    # mapped to 'emptyweight'
                ticket_data["loaded_weight"],   # mapped to 'loadedweight'
                ticket_data["net_weight"],      # mapped to 'netweight'
                ticket_data["material"],        # mapped to 'materialname'
                ticket_data["supplier"]         # mapped to 'suppliername'
            )
            execute_query(query, params)
            QMessageBox.information(self, "Success", "Ticket data saved successfully.")
        except Exception as e:
            QMessageBox.critical(self, "Database Error", f"Failed to save ticket data:\n{e}")

    def update_net_weight(self):
        try:
            loaded = int(self.loaded_weight.text()) if self.loaded_weight.text() else 0
            tare = int(self.empty_weight.text()) if self.empty_weight.text() else 0
            net = loaded - tare
            if loaded and tare:
