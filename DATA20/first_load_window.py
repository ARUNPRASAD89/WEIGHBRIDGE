...
from db_utils import execute_query, fetch_one, fetch_all, get_connection
...
class SummaryDialog(QDialog):
    ...
    def on_weightprint(self):
        try:
            ...
            wheel_label = "" # Not used in DB, but could be added if wanted

            eamount = blank_to_none(self.eamount)
            lamount = blank_to_none(self.lamount)
            tamount = blank_to_none(self.tamount)
            # Weight logic
        ...
        # For upsert/compatibility
            params = {
                ...
                "Pending": (empty_weight is not None and loaded_weight is None) or (loaded_weight is not None and empty_weight is None),
                "Closed": empty_weight is not None and loaded_weight is not None,
                "Exported": False,
                "Shift": "B",
                "Materialname": "",
                "SupplierName": "",
        ...
        # Prepare ticket_data for print/preview
            ticket_data = {
                "TicketNumber": f"{ticket_number:05d}",
                "VehicleNumber": vehicle_number,
                "Date": date,
                "Time": time,
                "EmptyWeight": empty_weight if empty_weight is not None else "",
                "LoadedWeight": loaded_weight if loaded_weight is not None else "",
                "NetWeight": net_weight if net_weight is not None else "",
                "Materialname": "",
                "SupplierName": "",
                "State": "first transaction",
                "AMOUNT": "",
                "STATUS": self.status,
                "EAMOUNT": eamount if eamount is not None else "",
                "LAMOUNT": lamount if lamount is not None else "",
                "TAMOUNT": tamount if tamount is not None else "",
                "NetWeight1": "",
                "LWEIGHT": "",
                "EWEIGHT": ""
            }
            self.ticket_data = ticket_data  # <--- ADD THIS LINE

            self.wp_btn.setVisible(False)
            self.success_label.setText(f"TICKET SAVED!")
            # Use your print prompt dialog/preview, or direct preview as you prefer:
            ...
            self.print_preview_timer.start(3000)
        except Exception as e:
...
class FirstLoadWindow(QWidget):
    ...
    def generate_ticket_number(self):
        # Always generate 5-digit ticket numbers starting from '00001'
        row = fetch_one(
            'SELECT COALESCE(MAX(CAST("TicketNumber" AS INTEGER)), 0) + 1 AS next_ticket FROM tickets'
        )
        next_ticket = row["next_ticket"] if row and "next_ticket" in row else 1
        if not next_ticket or next_ticket <= 0:
            next_ticket = 1
