...
from PyQt5.QtCore import Qt, QDate, QTime, QTimer, QLocale, QSize, QCoreApplication
...
def center_and_resize(widget, width_ratio=0.65, height_ratio=0.75, min_w=900, min_h=600):
    """
    Resize the widget as a percentage of the available screen geometry and center it.
    This respects the Windows taskbar area (availableGeometry).
    """
    screen = getattr(widget, "screen", None)() if hasattr(widget, "screen") else None
    if screen is None:
...
class SummaryDialog(QDialog):
    def __init__(self, parent, ticket_no, date, time, vehicle_number, weight, vehicle_type, status, eamount, lamount, tamount, transaction_window=None):
        ...
        label_font = QFont("Arial", 18, QFont.Bold)
        ...
        def add_info(row, col, label, value):
            ...
            val = QLabel(str(value)); val.setFont(label_font); val.setStyleSheet("color: black;")
    ...
    def showEvent(self, event):
        super().showEvent(event)
        center_and_resize(self, width_ratio=0.5, height_ratio=0.6, min_w=600, min_h=500)
...
class LoadStatusDialog(QDialog):
    def __init__(self, parent=None):
        ...
        # New: Highly visual, large colored buttons with unique symbols for drivers who can't read
        btn_layout = QHBoxLayout()
        btn_layout.setSpacing(40)

        # Use a big symbol line + multilingual labels. Colors and large fonts for easy recognition.
        self.empty_btn = QPushButton("○\nEMPTY[    >]\nஎமிட்டி\nखाली")
        self.load_btn  = QPushButton("▣\nLOAD[||||| >]\nலோடு\nलदान")

        # Styling and sizing for immediate recognition and easy tapping
        btn_font_symbol = QFont("Arial", 20, QFont.Bold)
        btn_font_text = QFont("Arial", 30, QFont.Bold)

        for btn, bg_color, fg_color, symbol_size in [
            (self.empty_btn, "#2E86C1", "white", btn_font_symbol),
            (self.load_btn,  "#C0392B", "white", btn_font_symbol)
        ]:
            # Set the font: symbol line will use a very large font and the rest inherits the button font size.
            btn.setFont(btn_font_text)
            btn.setMinimumSize(420, 300)
            # Large padding and rounded corners; use multi-line text, center-aligned.
            btn.setStyleSheet(
                f"QPushButton {{ background-color: {bg_color}; color: {fg_color}; "
                "border-radius: 20px; padding: 18px; font-weight: bold; }}"
                "QPushButton:pressed { background-color: #555555; }"
            )
            btn.setFocusPolicy(Qt.NoFocus)
            btn.setAccessibleName("LoadStatusButton")
            btn_layout.addWidget(btn)

        # Make the symbol line extra-large by setting an icon-like style: prepend large symbol using html is not reliable across platforms,
        # so we keep the symbol as first text line and ensure the overall button font is large enough for both symbol and text.
        self.empty_btn.setFont(QFont("Arial", 30, QFont.Bold))
        self.load_btn.setFont(QFont("Arial", 30, QFont.Bold))
    ...
    def showEvent(self, event):
        super().showEvent(event)
        center_and_resize(self, width_ratio=0.3, height_ratio=0.25, min_w=420, min_h=240)

    def set_result(self, status): self.result = status; self.accept()

# In first_load_window.py, replace the existing VehicleSelectionDialog class with this one.
...
class VehicleSelectionDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        self.result = None
        self.vehicle_options = []  # Store options to avoid re-querying

        main_layout = QVBoxLayout(self)
        label = QLabel("Select the Vehicle Type:")
        ...
        try:
            # --- FIX: The complete, correct logic for creating buttons with images ---
            ...
            for idx, vehicle_data in enumerate(self.vehicle_options):
                ...
                image_path = vehicle_data.get('image_path') # Get the image path

                # Create the button and set its properties
                btn = QPushButton(vehicle_name)
                btn.setFont(QFont("Arial", 12, QFont.Bold))
                btn.setCheckable(True)
                btn.setMinimumHeight(80) # Taller to accommodate image
                btn.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
                btn.setIconSize(QSize(140, 140)) # Set a good size for the icon

                # If an image path exists and the file is found, set it as the button's icon
...
class FirstLoadWindow(QWidget):
    def __init__(self, load_status, vehicle_type, mode_window=None):
        ...
        self._define_fonts(); self._setup_ui(); self._connect_signals(); self._initialize_state(); self._start_timers()
        # Center and size according to system resolution
        center_and_resize(self, width_ratio=0.65, height_ratio=0.75, min_w=1100, min_h=700)
        
        # --- NEW: SERIAL PORT INTEGRATION ---
        self.serial_manager = None
        if SERIAL_MANAGER_AVAILABLE:
            ...
            QMessageBox.warning(self, "Serial Manager Not Found", "Could not import SerialManager. Weight will not be live.")
    ...
    def _create_top_bar(self):
        ...
        weight_label = QLabel("Weight (KG):"); weight_label.setFont(self.font_label); self.weight_display = QLabel("0"); self.weight_display.setFont(self.font_weight); self.weight_display.setAlignment(Qt.AlignRight | Qt.AlignVCenter); self.weight_display.setStyleSheet("color:white; background:black; border-radius: 8px; padding: 4px 32px; min-width: 180px;")
    ...
    def _create_keyboard(self):
        ...
        clear_btn = QPushButton("Clear"); clear_btn.setFont(self.digit_font); clear_btn.setFixedSize(92, 52); clear_btn.setStyleSheet("background-color: red; color: white; font-weight: bold;"); clear_btn.clicked.connect(lambda: self.vehicle_input.setText(""))
    ...
    def update_live_weight(self, weight):
        """Updates the weight display and related placeholder fields."""
        self.weight_display.setText(weight)
        self.update_weight_placeholders()

    def show_serial_error(self, message):
        """Displays serial port errors to the user."""
        QMessageBox.warning(self, "Serial Port Error", message)

    def closeEvent(self, event):
        """Ensures the serial manager thread is stopped cleanly on window close."""
    ...
    def open_second_load_window(self, ticket_number, vehicle_number):
        try:
            ...
            # Close the current window and open the second-load window
            self.close()
            self.second_win = SecondLoadWindow()

            # Put the ticket number into the SecondLoadWindow's search box and make it readonly
            # SecondLoadWindow expects search_input (not ticket_number/vehicle_input).
            self.second_win.search_input.setText(str(ticket_number))
            self.second_win.search_input.setReadOnly(True)

            # Optionally store the expected vehicle number so later code can use it if needed
            # (not strictly required because ok_pressed() will reload the DB record).
            self.second_win.first_load_data = {"TicketNumber": ticket_number, "VehicleNumber": vehicle_number}

            # Let the SecondLoadWindow process the ticket as if OK was pressed.
            # Use a single-shot QTimer so processing happens after the window is created/shown.
            QTimer.singleShot(0, self.second_win.ok_pressed)
...
if __name__ == "__main__":
    # Enable High-DPI scaling and crisp icons on Windows and other high-DPI displays
    try:
