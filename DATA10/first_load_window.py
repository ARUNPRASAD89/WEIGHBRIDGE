...
class SummaryDialog(QDialog):
    def __init__(self, parent, ticket_no, date, time, vehicle_number, weight, wheel_label, status, eamount, lamount, tamount, transaction_window=None):
        ...
        btn_row = QHBoxLayout()
        self.wp_btn = QPushButton("WeightPrint")
    ...
    def on_weightprint(self):
        try:
            ...
            # PATCH: set Pending and Closed flags correctly
            if empty_weight and loaded_weight:
                pending = False
                closed = True
            else:
            ...
            insert_query = """
                INSERT INTO tickets (
                    ...
                    "EmptyWeight", "LoadedWeight", "NetWeight", "Pending", "Closed", "EAMOUNT", "LAMOUNT", "TAMOUNT"
            ...
            params = (
                ...
                pending, closed,
                eamount, lamount, tamount
            )
            execute_query(insert_query, params)
            self.wp_btn.setVisible(False)
            self.success_label.setText(f"TICKET SAVED SUCCESSFULLY ON {self.ticket_no}")
            QTimer.singleShot(5000, self._finish_and_goto_transaction)
        except Exception as e:
    ...
    def _finish_and_goto_transaction(self):
        ...
        if self.transaction_window:
            # Close parent window (FirstLoadWindow)
            parent = self.parent()
            if parent:
                parent.close()
            self.transaction_window.show()
        else:
            # As fallback, show parent if transaction_window is not set
            parent = self.parent()
            if parent:
                parent.show()
