...
def get_default_template_fields(conn):
    """
    Returns (templatename, ticket_width_mm, ticket_height_mm, copies, fields)

    Falls back to copies=1 if the copies column doesn't exist or any error occurs.
    """
    with conn.cursor() as cur:
        # Try to read copies if available; fall back if column missing
        try:
            cur.execute("SELECT templatename, ticketwidth, ticketheight, copies FROM templatemaster WHERE defaulttemplate = true;")
            ...
            try:
                copies = int(template[3]) if template[3] is not None else 1
            except Exception:
                copies = 1
        except Exception:
            # Column 'copies' may not exist or some other error; fallback to old query and copies=1
    ...
    return templatename, ticket_width_mm, ticket_height_mm, copies, fields
...
def print_ticket_with_template(data_dict, conn=None, printer_name=None, job_title="Weighbridge Ticket"):
    ...
    templatename, ticket_width_mm, ticket_height_mm, copies, fields = get_default_template_fields(conn)
    ...
    try:
        if any(looks_like_image_path(ticket_data.get(f["fieldname"])) for f in fields):
            ...
            # Render once per copy requested.
            for i in range(copies):
                this_title = f"{job_title} (copy {i+1} of {copies})"
                # render_ticket_with_data will honor DPI and font sizing; it accepts printer_name for silent printing
    ...
    # Win32 text-only printing (silent). Repeat document for number of copies.
    if not printer_name:
