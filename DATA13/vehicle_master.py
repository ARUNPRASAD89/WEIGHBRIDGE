...
class VehicleMaster(QWidget):
    ...
    def init_ui(self):
        ...
        # Exit button to return to configuration window
        self.exit_btn = QPushButton("Exit")
    ...
    def get_next_vehicle_id(self):
        row = fetch_one("SELECT MAX(vehicleid) AS max_id FROM vehiclemaster")
    ...
    def add_vehicle(self):
        ...
        # Create next vehicleid in sequence
        vehicle_id = self.get_next_vehicle_id()

        query = """
            INSERT INTO vehiclemaster (vehicleid, vehiclenumber, vehicletareweight)
            VALUES (%s, %s, %s)
        """
        params = (vehicle_id, vehicle_number, vehicle_tareweight if vehicle_tareweight else None)

        try:
            execute_query(query, params)
            QMessageBox.information(self, "Success", f"Vehicle added successfully with ID: {vehicle_id}!")
            self.populate_vehicle_id_combo()
            self.vehicle_number_edit.clear()
            self.vehicle_tareweight_edit.clear()
        except Exception as e:
            QMessageBox.critical(self, "Database Error", f"Error adding vehicle:\n{e}")

    def exit_to_config(self):
        self.close()
        from configuration_window import ConfigurationWindow  # <-- move here!
