...
class SummaryDialog(QDialog):
    ...
    def on_weightprint(self):
        try:
            ...
            insert_query = """
                INSERT INTO tickets (
                    "TicketNumber", "VehicleNumber", "Date", "Time", "EmptyWeight", "LoadedWeight", "NetWeight",
                    "Materialname", "SupplierName", "State", "EAMOUNT", "LAMOUNT", "TAMOUNT", "STATUS"
            ...
            """
            params = (
                self.ticket_no, self.vehicle_number, self.date, self.time,
                empty_weight or None, loaded_weight or None, net_weight or None,
                "", "", "", eamount or None, lamount or None, tamount or None, self.status
            )
            execute_query(insert_query, params)

            self.wp_btn.setVisible(False)
            self.success_label.setText(f"TICKET SAVED!")
            # 1. Show print preview for 5 seconds using the just-saved data
            # -- Fetch from DB --
            saved_row = fetch_one('SELECT * FROM tickets WHERE "TicketNumber" = %s', (self.ticket_no,))
            if saved_row:
                ticket_data = dict(saved_row)
            else:
                ticket_data = self.ticket_data  # fallback, should not happen
            ...
            self.print_preview_timer.timeout.connect(lambda: self._do_print_ticket(ticket_data))
            self.print_preview_timer.start(5000)

        except Exception as e:
            QMessageBox.critical(self, "Error", f"Could not save/print ticket:\n{e}")

    def _do_print_ticket(self, ticket_data=None):
        if self._preview_dialog:
            self._preview_dialog.close()
        self.success_label.setText("Printing ticket...")

        conn = get_connection()
        # Always fetch the ticket from DB by TicketNumber unless passed as argument
        if ticket_data is None:
            saved_row = fetch_one('SELECT * FROM tickets WHERE "TicketNumber" = %s', (self.ticket_no,))
            if saved_row:
                ticket_data = dict(saved_row)
            else:
                ticket_data = self.ticket_data  # fallback

        print_ticket_with_template(ticket_data, conn)
