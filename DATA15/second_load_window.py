...
class SummaryDialog(QDialog):
    ...
    def on_weighprint(self):
        try:
            ...
            # 2. Fetch updated ticket row from DB
            saved_row = fetch_one('SELECT * FROM tickets WHERE "TicketNumber" = %s', (self.data.get("TicketNumber", None),))
            ticket_data = dict(saved_row) if saved_row else self.data

            # 3. Show Preview Dialog
            self._preview_dialog = TicketPreviewDialog(ticket_data, parent=self)
            self._preview_dialog.show()

            # 4. After delay, print the ticket
            self._print_timer = QTimer(self)
            self._print_timer.setSingleShot(True)
            self._print_timer.timeout.connect(lambda: self._do_print_ticket(ticket_data))
            self._print_timer.start(5000)
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Could not save/print ticket:\n{e}")

    def _do_print_ticket(self, ticket_data):
        ...
        # 5. After print, return to transaction window
