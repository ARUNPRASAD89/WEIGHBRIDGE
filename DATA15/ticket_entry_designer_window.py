from PyQt5.QtWidgets import (
    ...
    QFrame, QFontDialog, QApplication, QMenu, QInputDialog, QMessageBox, QCheckBox
)
...
class TicketEntryDesignerWindow(QWidget):
    def __init__(self):
        ...
        for text in ["New", "Open", "Delete", "Save", "Print Preview", "Help", "Exit"]:
            btn = QPushButton(text)
            btn.setFixedWidth(90)
            self.left_btn_layout.addWidget(btn)
            if text == "New":
                btn.clicked.connect(self.new_template)
            if text == "Save":
            ...
            if text == "Print Preview":
                btn.clicked.connect(self.preview_print)
    ...
    def insert_field(self):
        ...
        # Optional: Prevent duplicate fields by name in canvas
        # if any(f.field_name == field_name for f in self.canvas.fields):
        #     QMessageBox.warning(self, "Duplicate Field", f"The field '{field_name}' is already added.")
        #     return
    ...
    def save_template(self):
        ...
        # Only keep the last instance of each field_name (optional: remove for true duplicates)
        # unique_fields = {}
        # for f in self.canvas.fields:
        #     unique_fields[f.field_name] = f
    ...
    def load_default_template(self):
        ...
        if default:
            self.template_name_edit.setText(default[0])
            self.load_template()
    ...
    def load_template(self):
        ...
        if template:
            ...
            self.update_canvas_size()
            self.set_default_checkbox.setChecked(bool(template[3]))
